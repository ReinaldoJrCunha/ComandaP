...
$query = "SELECT Codigo_produto, Vendas FROM registros WHERE Data='$data_atual'";
...
if ($result = $mysqli->query($query)) {
	if ($result->num_rows > 0) { //Verifca se foram retornados resultados
    	while ($row = $result->fetch_assoc()) {
			...
			#cho("Produto ".$row["Codigo_produto"].' - Vendas '.$row["Vendas"].' - ');
    	    ...
    	    if ($result2 = $mysqli2->query($query2)) {
				if ($result2->num_rows > 0) { //Verifca se foram retornados resultados
			    	...
			    	$valor_total_das_vendas += $quantidade_vendas*$atributos_indice[0];
